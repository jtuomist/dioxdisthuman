...
#      muind[i,1:C] <- mu[fis[i],1:C]

      for(i in 1:I) { 
        for(y in 1:length(YEAR)) {
          mu[i,y] <- a[s[i],1]*(YEAR[y]+age[i])^2 + a[s[i],2]*(YEAR[y]+age[i]) + a[s[i],3]
          fat[i,y] ~ dnorm(mu[i,y] , tau[s[i]])
          pred[i,y] ~ dnorm(mu[i,y] , tau[s[i]])
        }
      }
    }
  ")
  ...
  samps.j <- jags.samples(
    jags, 
    variable.names=c("a","tau", "mu","pred"),
    n.iter=N*10,
    thin=10
  )
  ...
  scatterplotMatrix(t(samps.j$tau[,,1]), main="Precision for girls and boys")
