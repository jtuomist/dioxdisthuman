...
B <- matrix(rep(0,length(AGE)*length(INDIVIDUAL)),ncol=length(AGE))
  ...
  mod <- textConnection(
    "
    var k, a, b, U, E[61], C[5,H], B[5,H], M[5,H], C1, C2, conc[5,2], SYNT[5],
...
#      B[,1] <- B[,1]*0
#      for(y in 2:length(AGE)) {
#        B[,y] <- B[,y-1] * (1 - k) + U * A * W[,y-1] * E[SYNT[]-1960+y-1] * P[y-1]
#      }
      ...
      for(i in 1:I) { 
        conc[i,1] ~ dnorm(C[i,1980 - SYNT[i]+1], 0.01) # Age 0 is at position 1
...
#      AGE = AGE,
      ...
      M = M[1:5,1:H],
      H = H,
      I = 5,#length(INDIVIDUAL),
      U = U,
      A = A,
      b = b,
      c = c,
      P = P[1:H],
      SYNT = df$SYNTVUOSI[1:5],
