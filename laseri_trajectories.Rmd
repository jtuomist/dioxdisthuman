...
df <- df[df$SP=="Poika", c("PCB15380_norm","PCB15301_norm","SYNTVUOSI", "BMI80","BMI01","Paino01")] # ,"bodyfat80","bodyfat92"
df <- na.omit(df)
...
k <- 0.22 # 1/a, elimination constant of the POP
a <- 1.6 # ug/kg/d, average intake at the peak exposure
N <- 100
b <- 0.07 # 1/a, rate of decrease of the POP in the environment and intake
c <- 1900 # year of peak exposure
...
ggplot(aggregate(long["PCB"], by=long[c("SYNTVUOSI","year")], FUN=mean),
       aes(x=year, y=PCB, colour=SYNTVUOSI, group=SYNTVUOSI))+geom_line()

long$age <- long$year - long$SYNTVUOSI
ggplot(aggregate(long["PCB"], by=long[c("SYNTVUOSI","age")], FUN=mean),
...
ggplot(aggregate(long["BMI"], by=long[c("SYNTVUOSI","age")], FUN=mean),
...
#      b ~ dnorm(0.1, 0.001)
  ...
  jags <- jags.model(
    ...
    data = list(
      ...
      P = P[1:H],
      SYNT = SYNT,
      conc = df[1:I,c("PCB15380_norm","PCB15301_norm")],
      B1 = rep(0,I)
    ),
    n.chains = 4,
    n.adapt = 100
  ...
  cat("Average exposure during peak year (ug/kg/d)\n")
  plot(coda.samples(jags, 'a', N))
  cat("Exposure reduction rate (1/a)\n")
  plot(coda.samples(jags, 'b', N))
  cat("Exposure reduction rate (1/a)\n")
#  plot(coda.samples(jags, 'E', N))
...
tst <- data.frame(PCB15380_pred = tst[,1],PCB15301_pred = tst[,2],
                  df[c("PCB15380_norm","PCB15301_norm")])
pairs.panels(tst)
